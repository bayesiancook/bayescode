...
class Product : public Selector<double>  {
    ...
    Product(const Selector<double>& inbranchval, double inmulval) : branchval(inbranchval), mulval(inmulval), array(inbranchval.GetSize()) {}
    ~Product() {}
 ...
 *
 * Used in OmegaModel: the mean of omega_gj, for gene g and branch j, is equal to w_g * branchv_j
 */
...
class ProductArray : public Array<Product>    {
    ...
    ProductArray(const Selector<double>& inbranchval, const Selector<double>& insiteval) : branchval(inbranchval), siteval(insiteval), array(insiteval.GetSize(), (Product*) 0) {
    ...
    Product& operator[](int gene)  {
        return *array[gene];
    }

    //! global update of the arrays (needs to be called each time the array has changed)
    void Update()   {
        for (int gene=0; gene<GetSize(); gene++)    {
            array[gene]->SetMulVal(siteval.GetVal(gene));
        }
