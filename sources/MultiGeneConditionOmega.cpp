...
class MultiGeneConditionOmegaChain : public MultiGeneChain  {
  ...
  public:
    ...
    MultiGeneConditionOmegaChain(string indatafile, string intreefile, int inncond, int innlevel, int inevery, int inuntil, string inname, int force, int inmyid, int innprocs) : MultiGeneChain(inmyid,innprocs), modeltype("MULTIGENECONDOMEGA"), datafile(indatafile), treefile(intreefile), ncond(inncond), nlevel(innlevel) {
        every = inevery;
        until = inuntil;
        name = inname;
        New(force);
    ...
    MultiGeneConditionOmegaChain(string filename, int inmyid, int innprocs) : MultiGeneChain(inmyid,innprocs) {
        name = filename;
        Open();
	Save();
    }
...
int main(int argc, char* argv[])	{
    ...
    // new chain
        ...
        chain = new MultiGeneConditionOmegaChain(datafile,treefile,ncond,nlevel,every,until,name,force,myid,nprocs);
    }
