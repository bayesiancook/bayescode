...
/**
 * \brief An interface for an abstract indexed bidim-array of constant references over objects of type T
 *
 * This abstract class is meant as a general read-only interface
 * returning a reference over a T object for any pair of indices (through the GetVal(int i, int j) method).
 * As an example, DiffSelSparseModel implements a bidim array of site- and condition-specific fitness vectors over the 20 amino acids (fitness vector)
 */
 ...
 *
 * Deriving from BidimSelector, BidimArray gives const access through GetVal(int i, int j).
 ...
 * The SimpleBidimArray class template provides a more explicit implementation (by encapsulating a vector<vector<T> >, see SimpleBidimArray).
 * However, in some cases, it can be useful to implement a BidimArray<T> as a vector<vector<T*> > instead, hence the specific 
...
/**
 * \brief A BidimSelector<T> that returns a reference to the same value T for any pair of indices (i,j)
 */

template<class T> class BidimHomogeneousSelector : public BidimSelector<T>	{

	public:
    //! \brief Constructor, taking as its arguments the number of rows and columns of the array and the value to be returned for any pair of indices
...
/**
 * \brief The 'standard' implementation of a BidimArray<T>, simply as a vector<vector<T> >
 *
 */
