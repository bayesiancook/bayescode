...
class BranchSpecificMeanGammaTree: public SimpleBranchArray<double>	{
	...
	BranchSpecificMeanGammaTree(const BranchSelector<double>& inblmean, double ininvshape) : SimpleBranchArray<double>(inblmean.GetTree()), blmean(inblmean), invshape(ininvshape)	{
		Sample();
	}
...
class BranchSpecificMeanGammaTreeArray : public Array<BranchSpecificMeanGammaTree>    {
    ...
    BranchSpecificMeanGammaTreeArray(const BranchProductArray& inmean, double ininvshape) : mean(inmean), invshape(ininvshape), array(inmean.GetSize(), (BranchSpecificMeanGammaTree*) 0)    {
    ...
    const BranchSpecificMeanGammaTree& GetVal(int gene) const {
        return *array[gene];
    }

    //! non-const access for the given gene
    BranchSpecificMeanGammaTree& operator[](int gene)  {
        return *array[gene];
    }
