...
class CodonSequenceAlignment : public SequenceAlignment {
  public:
    ...
    //!
    //! If force_stops is false, returns an error message when encountering stop codons -- otherwise, replace stop codons by missing entries.
    //! If any codon has any missing entry in the three nucleotide positions, then the whole codon is considered missing.
    ...
    CodonStateSpace *GetCodonStateSpace() {
        return (CodonStateSpace *)(GetStateSpace());
    }
