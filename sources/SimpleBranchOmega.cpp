...
class BranchOmegaChain : public Chain  {
  ...
  public:
    ...
    BranchOmegaChain(string indatafile, string intreefile, int inevery, int inuntil, string inname, int force) : modeltype("BRANCHOMEGA"), datafile(indatafile), treefile(intreefile) {
        every = inevery;
        until = inuntil;
        name = inname;
        New(force);
    ...
    BranchOmegaModel* GetModel() {
        return static_cast<BranchOmegaModel*>(model);
    }
...
int main(int argc, char* argv[])	{
    ...
    // new chain
        ...
        BranchOmegaChain* chain = new BranchOmegaChain(datafile,treefile,every,until,name,force);
        cerr << "chain " << name << " started\n";
        chain->Start();
        cerr << "chain " << name << " stopped\n";
        cerr << chain->GetSize() << "-- Points saved, current ln prob = " << chain->GetModel()->GetLogProb() << "\n";
