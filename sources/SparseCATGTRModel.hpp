...
class SparseCATGTRModel : public ProbModel {
  ...
  public:
    SparseCATGTRModel(
        const string &datafile, const string &treefile, int inratemode, double inepsilon)
        : rrsuffstat(Naa) {
        blmode = 0;
        rrmode = 0;
        ratemode = inratemode;
    ...
    //! copy relative exchange rates into vectors given as arguments (multi-gene
    //! analyses)
    void GetRelRates(vector<double> &inrelrate) const { inrelrate = relrate; }
    ...
    void CorruptMatrices() { sitematrixarray->UpdateMatrices(); }
    ...
    double SiteSuffStatLogProb(int i) const {
        // return
        // sitepathsuffstatarray->GetVal(i).GetLogProb(sitematrixarray->GetVal(i));
    ...
    void MoveParameters(int nrep0, int nrep) {
        for (int rep0 = 0; rep0 < nrep0; rep0++) {
            ...
            for (int rep = 0; rep < nrep; rep++) {
                ...
                MovePreProfile();
                CompMovePreProfile();
                if (maskmode < 3) { MoveMasks(); }
                if (maskmode < 2) { MoveMaskHyperParameters(); }
                if (maskepsilonmode < 2) { MoveMaskEpsilon(); }
            }
            if (rrmode < 2) {
                MoveRelRates();
    ...
    void MoveLambda() {
        ...
        ScalingMove(lambda, 1.0, 10, &SparseCATGTRModel::BranchLengthsHyperLogProb,
            &SparseCATGTRModel::NoUpdate, this);
        ScalingMove(lambda, 0.3, 10, &SparseCATGTRModel::BranchLengthsHyperLogProb,
            &SparseCATGTRModel::NoUpdate, this);
        blhypermean->SetAllBranches(1.0 / lambda);
    }
    ...
    void MoveAlpha() {
        ...
        ScalingMove(alpha, 1.0, 10, &SparseCATGTRModel::SiteRateHyperLogProb,
            &SparseCATGTRModel::NoUpdate, this);
        ScalingMove(alpha, 0.3, 10, &SparseCATGTRModel::SiteRateHyperLogProb,
            &SparseCATGTRModel::NoUpdate, this);
        siterate->SetShape(alpha);
        siterate->SetScale(alpha);
    }
    ...
    void CompMovePreProfile() { CompMovePreProfile(1.0, 10); }
    ...
    void MoveMaskHyperParameters() {
        SlidingMove(pi, 1.0, 10, 0.05, 0.975, &SparseCATGTRModel::MaskLogProb,
            &SparseCATGTRModel::UpdateMask, this);
        SlidingMove(pi, 0.1, 10, 0.05, 0.975, &SparseCATGTRModel::MaskLogProb,
            &SparseCATGTRModel::UpdateMask, this);
    }

    //! MH move schedule on background fitness (maskepsilon)
    void MoveMaskEpsilon() {
        SlidingMove(maskepsilon, 1.0, 10, 0, 1.0, &SparseCATGTRModel::MaskEpsilonLogProb,
            &SparseCATGTRModel::UpdateAll, this);
        SlidingMove(maskepsilon, 0.1, 10, 0, 1.0, &SparseCATGTRModel::MaskEpsilonLogProb,
            &SparseCATGTRModel::UpdateAll, this);
    }
