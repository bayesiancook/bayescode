...
template<class T> class BranchMixtureSelector : public BranchSelector<T>	{
	...
	BranchMixtureSelector(const Selector<T>& incomponents, const BranchAllocationSystem& inalloc) : components(incomponents), alloc(inalloc)	{
	}
	...
	const T& GetVal(int index) const override {return components.GetVal(alloc.GetBranchAlloc(index));}
...
class ConditionOmegaModel : public ProbModel {
    ...
    */
	...
	void Allocate()	{
		...
		codonmatrixarray = new MGOmegaCodonSubMatrixArray(GetCodonStateSpace(), nucmatrix, condomegaarray);
		...
		phyloprocess = new PhyloProcess(tree,codondata,branchlength,0,submatrixtree,rootcodonmatrix);
    ...
    ConditionSpecificMeanGammaArray* GetOmegaArray() const  {
        return condomegaarray;
    }
    ...
    void SetCondV(const Selector<double>& incondv)    {
        condv->Copy(incondv);
    }
	...
	double OmegaLogPrior()	const {
        return condomegaarray->GetLogProb();
	}
	...
	void MoveBranchLengthsHyperParameter()	{
        ...
        ScalingMove(lambda,1.0,10,&ConditionOmegaModel::BranchLengthsHyperLogProb,&ConditionOmegaModel::NoUpdate,this);
        ScalingMove(lambda,0.3,10,&ConditionOmegaModel::BranchLengthsHyperLogProb,&ConditionOmegaModel::NoUpdate,this);
		branchlength->SetScale(lambda);
	}
	...
	void MoveNucRates()	{
        ...
        ProfileMove(nucrelrate,0.1,1,3,&ConditionOmegaModel::NucRatesLogProb,&ConditionOmegaModel::TouchNucMatrix,this);
        ProfileMove(nucrelrate,0.03,3,3,&ConditionOmegaModel::NucRatesLogProb,&ConditionOmegaModel::TouchNucMatrix,this);
        ProfileMove(nucrelrate,0.01,3,3,&ConditionOmegaModel::NucRatesLogProb,&ConditionOmegaModel::TouchNucMatrix,this);

        ProfileMove(nucstat,0.1,1,3,&ConditionOmegaModel::NucRatesLogProb,&ConditionOmegaModel::TouchNucMatrix,this);
        ProfileMove(nucstat,0.01,1,3,&ConditionOmegaModel::NucRatesLogProb,&ConditionOmegaModel::TouchNucMatrix,this);
