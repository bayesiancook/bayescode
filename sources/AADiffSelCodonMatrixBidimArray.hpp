...
class AADiffSelCodonMatrixBidimArray : public BidimArray<SubMatrix>, public BidimArray<AAMutSelOmegaCodonSubMatrix>  {
    ...
    AADiffSelCodonMatrixBidimArray(const BidimSelector<vector<double> >& infitnessarray, const CodonStateSpace& incodonstatespace, const SubMatrix& innucmatrix) :
        fitnessarray(infitnessarray), codonstatespace(incodonstatespace), nucmatrix(innucmatrix),
        matrixarray(infitnessarray.GetNrow(),vector<AAMutSelOmegaCodonSubMatrix*>(infitnessarray.GetNcol(),(AAMutSelOmegaCodonSubMatrix*)0)) {
        Create();
    }

    ~AADiffSelCodonMatrixBidimArray()   {
        Delete();
    }
    ...
    void Create()   {
        for (int i=0; i<GetNrow(); i++)  {
            for (int j=0; j<GetNcol(); j++)   {
                matrixarray[i][j] = new AAMutSelOmegaCodonSubMatrix(&codonstatespace,&nucmatrix,fitnessarray.GetVal(i,j),1.0,1.0);
            }
        }
    }
