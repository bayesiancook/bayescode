...
void AAMutSelSubMatrix::ComputeArray(int a) const {
    ...
    for (int i=0; i<codonstatespace->GetNstate(); i++)  {
        if (codonstatespace->Translation(i) == a)   {
            ...
            double weight = nucmatrix->Stationary(codonstatespace->GetCodonPosition(0,i)) * nucmatrix->Stationary(codonstatespace->GetCodonPosition(1,i)) * nucmatrix->Stationary(codonstatespace->GetCodonPosition(2,i)) * GetFitness(a);
            totweight += weight;
