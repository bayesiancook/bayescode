...
public:
    template<class... Args>
    ChainCmdLine(int argc, char *argv[], Args&&... args) : cmd(std::forward<Args>(args)...), argc(argc), argv(argv) {
    }
    ...
    std::ifstream checkpoint_file() {
        return std::ifstream(chain_name() + ".param");
    }

    CmdLine& get() { return cmd; }

    std::string chain_name() {
        if(resume_from_checkpoint()) return std::string(argv[1]);
        else if(already_parsed) return chain_name_arg.getValue();
        else {
