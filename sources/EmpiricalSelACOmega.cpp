#include <cmath>
#include <fstream>
#include "EmpiricalSelACOmegaModel.hpp"
#include "Chain.hpp"
using namespace std;

/**
 * \brief Chain object for running an MCMC under EmpiricalSelACOmegaModel
 */

class EmpiricalSelACOmegaChain : public Chain {
  private:
    // Chain parameters
    string modeltype, datafile, treefile, aafitnessfile;
    int omegamode, omegaprior;
    double dposompi, dposomhypermean, dposomhyperinvshape;
    int Gcat;

  public:
    EmpiricalSelACOmegaModel *GetModel() { return static_cast<EmpiricalSelACOmegaModel *>(model); }

    string GetModelType() override { return modeltype; }

    EmpiricalSelACOmegaChain(string indatafile, string intreefile, string inaafitnessfile, int inomegamode, int inomegaprior,
    ...
    EmpiricalSelACOmegaChain(string filename) {
        name = filename;
        Open();
        Save();
    }
    ...
    void New(int force) override {
        cerr << "new model\n";
        model =
            new EmpiricalSelACOmegaModel(datafile, treefile, aafitnessfile, omegamode, omegaprior, Gcat);
        if (omegaprior == 1) {
    ...
    void Open() override {
        ...
        if (modeltype == "AAMUTSELDSBDPOMEGA") {
            model = new EmpiricalSelACOmegaModel(datafile, treefile, aafitnessfile, omegamode, omegaprior, Gcat);
            if (omegaprior == 1) {
...
int main(int argc, char *argv[]) {
    string name = "";
    EmpiricalSelACOmegaChain *chain = 0;

    // starting a chain from existing files
    if (argc == 2 && argv[1][0] != '-') {
        name = argv[1];
        chain = new EmpiricalSelACOmegaChain(name);
    }
    ...
    // new chain
        ...
        try {
            ...
            cerr << "empselac -d <alignment> -t <tree> -aa <aafile> -gcat <gcat> <chainname> \n";
            cerr << '\n';
            exit(1);
        }

        chain = new EmpiricalSelACOmegaChain(datafile, treefile, aafitnessfile, omegamode, omegaprior, dposompi,
