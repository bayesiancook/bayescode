 ...
 *
 * The array takes a single nucleotide matrix, an array of amino-acid fitness profiles and either a single omega or an array
 * of omega values and construct mutation-selection matrices accordingly.
 */

class AAMutSelOmegaCodonSubMatrixArray : public Array<SubMatrix>, public Array<AAMutSelOmegaCodonSubMatrix> {
  public:
    //! constructor with a nucleotide matrix, an array of amino-acid fitness profiles and a single omega value (for all
    //! matrices)
    AAMutSelOmegaCodonSubMatrixArray(const CodonStateSpace* incodonstatespace, const SubMatrix* innucmatrix,
                                     const Selector<vector<double> >* inaafitnessarray, double inomega)
        : codonstatespace(incodonstatespace),
          nucmatrix(innucmatrix),
    ...
    //! constructor with a nucleotide matrix, an array of amino-acid fitness profiles and an array of omega value (one for
    //! each entry of the matrix array)
    AAMutSelOmegaCodonSubMatrixArray(const CodonStateSpace* incodonstatespace, const SubMatrix* innucmatrix,
                                     const Selector<vector<double> >* inaafitnessarray, const Selector<double>* inomegaarray)
        : codonstatespace(incodonstatespace),
          nucmatrix(innucmatrix),
          aafitnessarray(inaafitnessarray),
          omegaarray(inomegaarray),
        ...
        if (aafitnessarray->GetSize() != omegaarray->GetSize()) {
            cerr << "error in constructor of AAMutSelOmegaCodonSubMatrixArray: arrays of aafitness and omega values should "
                    "be of same size\n";
            exit(1);
        }
        Create();
