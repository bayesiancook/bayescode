...
class AAMutSelCodonMatrixArray : public BidimArray<SubMatrix>, public BidimArray<AAMutSelOmegaCodonSubMatrix> {
  public:
    AAMutSelCodonMatrixArray(const BidimSelector<vector<double> >& infitnessarray, const CodonStateSpace& incodonstatespace,
    ...
    ~AAMutSelCodonMatrixArray() { Delete(); }
    ...
    void Create() {
        for (int i = 0; i < GetNrow(); i++) {
            for (int j = 0; j < GetNcol(); j++) {
                matrixarray[i][j] =
                    new AAMutSelOmegaCodonSubMatrix(&codonstatespace, &nucmatrix, fitnessarray.GetVal(i, j), 1.0, false);
            }
        }
    }
