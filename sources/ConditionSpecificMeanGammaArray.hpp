...
class ConditionSpecificMeanGammaArray: public SimpleArray<double>	{
	...
	ConditionSpecificMeanGammaArray(const Selector<double>& inblmean, double ininvshape) : SimpleArray<double>(inblmean.GetSize()), blmean(inblmean), invshape(ininvshape)	{
		Sample();
	}
...
class ConditionSpecificMeanGammaBidimArray : public Array<ConditionSpecificMeanGammaArray>    {
    ...
    ConditionSpecificMeanGammaBidimArray(const ProductArray& inmean, double ininvshape) : mean(inmean), invshape(ininvshape), array(inmean.GetSize(), (ConditionSpecificMeanGammaArray*) 0)    {
    ...
    const ConditionSpecificMeanGammaArray& GetVal(int gene) const {
        return *array[gene];
    }

    //! non-const access for the given gene
    ConditionSpecificMeanGammaArray& operator[](int gene)  {
        return *array[gene];
    }
