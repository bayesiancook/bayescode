...
class ProbModel {
  public:
    ...
    //! make a complete cycle of MCMC moves -- in principle, should return average success rate (although rarely does so in
    //! practice)
    ...
    template <class C>
    double SlidingMove(double& x, double tuning, int nrep, double min, double max, LogProbF<C> logprobf, UpdateF<C> updatef,
                       C* This) {
    ...
    //! Returns success rate.
    template <class C>
    double ScalingMove(double& x, double tuning, int nrep, LogProbF<C> logprobf, UpdateF<C> updatef, C* This) {
        double nacc = 0;
        double ntot = 0;
        for (int rep = 0; rep < nrep; rep++) {
            double deltalogprob = -(This->*logprobf)();
    ...
    //! Returns success rate.
    template <class C>
    double ProfileMove(vector<double>& x, double tuning, int n, int nrep, LogProbF<C> logprobf, UpdateF<C> updatef,
                       C* This) {
        double nacc = 0;
        double ntot = 0;
