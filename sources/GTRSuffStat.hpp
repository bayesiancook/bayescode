...
class RelRateSuffStat : public SuffStat {
    ...
    void AddSuffStat(const GTRSubMatrix& matrix, const PathSuffStat& pathsuffstat, double rate = 1)  {
	...
	void AddSuffStat(const GTRSubMatrixArray& matrixarray, const PathSuffStatArray& pathsuffstatarray)    {
...
class ProfileSuffStat : public SuffStat {
    ...
    ProfileSuffStat(int innstate) : nstate(innstate), profilecount(innstate,0), profilebeta(innstate,0) {}
    ~ProfileSuffStat() {}
...
class ProfileSuffStatArray: public SimpleArray<ProfileSuffStat>  {
	...
	void AddSuffStat(const Selector<GTRSubMatrix>& matrixarray, const Selector<PathSuffStat>& pathsuffstatarray)	{
		for (int i=0; i<GetSize(); i++)	{
            (*this)[i].AddSuffStat(matrixarray.GetVal(i),pathsuffstatarray.GetVal(i));
		}
	}

	void AddSuffStat(const Selector<GTRSubMatrix>& matrixarray, const Selector<PathSuffStat>& pathsuffstatarray, const Selector<double>& rate)	{
		for (int i=0; i<GetSize(); i++)	{
            (*this)[i].AddSuffStat(matrixarray.GetVal(i),pathsuffstatarray.GetVal(i),rate.GetVal(i));
		}
	}
