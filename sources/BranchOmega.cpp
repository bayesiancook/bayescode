...
/**
 * \brief Chain object for running an MCMC under BranchOmegaModel
 */
...
class BranchOmegaChain : public Chain {
  ...
  public:
    ...
    BranchOmegaChain(string indatafile, string intreefile, int inevery, int inuntil, string inname,
                     int force)
        : modeltype("BRANCHOMEGA"), datafile(indatafile), treefile(intreefile) {
        every = inevery;
        until = inuntil;
        name = inname;
        New(force);
    ...
    BranchOmegaModel *GetModel() { return static_cast<BranchOmegaModel *>(model); }
...
int main(int argc, char *argv[]) {
    string name = "";
    BranchOmegaChain *chain = 0;

    // starting a chain from existing files
    if (argc == 2 && argv[1][0] != '-') {
        name = argv[1];
        chain = new BranchOmegaChain(name);
    }
    ...
    // new chain
        ...
        chain = new BranchOmegaChain(datafile, treefile, every, until, name, force);
    }
