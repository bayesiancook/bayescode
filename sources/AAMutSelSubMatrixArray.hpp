...
class AAMutSelSubMatrixArray : public Array<SubMatrix>, public Array<AAMutSelSubMatrix>	{
	...
	public:
    AAMutSelSubMatrixArray(const CodonStateSpace *incodonstatespace, const SubMatrix *innucmatrix, double inxi, const Selector<vector<double> >* inprofilearray, int innormalise) : codonstatespace(incodonstatespace), nucmatrix(innucmatrix), xi(inxi), profilearray(inprofilearray), normalise(innormalise), matrixarray(inprofilearray->GetSize()) {
        Create();
	}

	~AAMutSelSubMatrixArray()	{
        Delete();
	}
    ...
    void Create()   {
		for (int i=0; i<GetSize(); i++)	{
            matrixarray[i] = new AAMutSelSubMatrix(codonstatespace,nucmatrix,xi,profilearray->GetVal(i),1.0,normalise);
		}
    }
