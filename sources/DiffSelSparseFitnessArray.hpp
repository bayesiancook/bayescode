...
/**
 * \brief An array of site- and condition-specific fitness profiles -- sparse version
 *
...
/**
 * \brief An array of site- and condition-specific fitness profiles -- doubly sparse version
 *
 * this array is used in DiffSelDoublySparseModel; It implements two levels of sparsity, as follows:
 ...
 * - F_kia is the actual fitness of amino acid a at site i and under condition k (for k=0..K-1)
 *
 * In words, and compared to DiffSelSparseFitnessArray, the baseline profiles are first masked for some amino-acids
 * (which are then considered as low-fitness amino-acids across all conditions).
 * Then, dor non-baseline conditions k=1..K, if d_kia == 0, the fitness of amino-acid a at site i under condition k is just the (masked) baseline F_0ia;
 * otherwise, it is a 'new' fitness parameter, such as defined by G_kia.
 *
 * Note that, when Nlevel == 2, the relation between G, d and F unfolds over two levels:
 * - F_0ia = G_0ia * m_ia + epsilon * (1-m_ia)
 ...
 *
 * This array is used in AAMutSelSparseOmegaModel. It implements sparse fitness profiles across sites by masking some amino-acids, according to an array of binary variables (masks), at each site. Specifically, the fitness at site i for amino-acid a is given by:
...
class MutSelSparseFitnessArray : public SimpleArray<vector<double> >    {
    ...
    //! constructor, parameterized by input fitness array, mask array and epsilon (background fitness of low-fitness amino-acids)
    MutSelSparseFitnessArray(const Selector<vector<double> >& infitness, const Selector<vector<int> >& inmask, double inepsilon) : 
        SimpleArray<vector<double> >(infitness.GetSize(),vector<double>(infitness.GetVal(0).size(),0)),
        fitness(infitness), mask(inmask), epsilon(inepsilon)  {
            Update();
    }
