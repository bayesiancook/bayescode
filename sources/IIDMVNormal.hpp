...
/**
 * \brief A BidimArray of iid vectors (of dimension dim) of normal random variates.
 *
 * The Nrow*Ncol array is parameterized by a vector of variance parameters (var, of dimension Nrow).
 ...
 * and (x_ijk)_k=1..20 is the vector of differential selection effects over the 20 amino-acids,
 * for site j and under condition i (compared to reference condition).
 */
...
class BidimIIDMVNormal : public SimpleBidimArray<vector<double> >    {
    ...
    //! \brief constructor, parameterized by number of columns, dimension of vectors and vector of variance parameters
    //!
    //! number of rows is defined implicitly: equal to dimension of the vector of variance parameters.
    ...
    //! return mean (over columns, or sites) of variance (across rows, or conditions) for component (or amino-acid) k.
    ...
    //! return variance (across rows, or conditions) for component (or amino-acid) k.
    ...
    //! return log prob over column j, counting only those entries that are flagged
