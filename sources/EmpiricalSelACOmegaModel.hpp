...
class EmpiricalSelACOmegaModel : public ProbModel {
  ...
  public:
    ...
    EmpiricalSelACOmegaModel(string datafile, string treefile, string aafitnessfile, int inomegamode, int inomegaprior,
                            int inGcat) {
        blmode = 0;
        nucmode = 0;
	...
	empweight.assign(Naa,0);
	aafitness.assign(Naa, vector<double>(Naa,0));
	ifstream is(aafitnessfile.c_str());
	int naa;
	is >> naa;
	if (naa != Naa)	{
		cerr << "error when opening aafitness file\n";
		exit(1);
	}
	for (int i=0; i<Naa; i++)	{
		is >> empweight[i];
    ...
    EmpiricalSelACOmegaModel(const CodonSequenceAlignment* incodondata, const Tree* intree, const vector<double> inempweight, const vector<vector<double>> inaafitness, int inomegamode, int inomegaprior,
                            int inGcat) {
        blmode = 0;
        nucmode = 0;
        omegamode = inomegamode;
	...
	empweight = inempweight;
        aafitness = inaafitness;
    ...
    void MoveLambda() {
        ...
        ScalingMove(lambda, 1.0, 10, &EmpiricalSelACOmegaModel::LambdaHyperLogProb,
                    &EmpiricalSelACOmegaModel::NoUpdate, this);
        ScalingMove(lambda, 0.3, 10, &EmpiricalSelACOmegaModel::LambdaHyperLogProb,
                    &EmpiricalSelACOmegaModel::NoUpdate, this);
        blhypermean->SetAllBranches(1.0 / lambda);
    }
    ...
    void MoveNucRates() {
        ProfileMove(nucrelrate, 0.1, 1, 3, &EmpiricalSelACOmegaModel::NucRatesLogProb,
                    &EmpiricalSelACOmegaModel::UpdateMatrices, this);
        ProfileMove(nucrelrate, 0.03, 3, 3, &EmpiricalSelACOmegaModel::NucRatesLogProb,
                    &EmpiricalSelACOmegaModel::UpdateMatrices, this);
        ProfileMove(nucrelrate, 0.01, 3, 3, &EmpiricalSelACOmegaModel::NucRatesLogProb,
                    &EmpiricalSelACOmegaModel::UpdateMatrices, this);

        ProfileMove(nucstat, 0.1, 1, 3, &EmpiricalSelACOmegaModel::NucRatesLogProb,
                    &EmpiricalSelACOmegaModel::UpdateMatrices, this);
        ProfileMove(nucstat, 0.01, 1, 3, &EmpiricalSelACOmegaModel::NucRatesLogProb,
                    &EmpiricalSelACOmegaModel::UpdateMatrices, this);
    }

    double MoveGinvshape()  {
        G_acc[0] += ScalingMove(Ginvshape, 1.0, 3, &EmpiricalSelACOmegaModel::AALogProb, &EmpiricalSelACOmegaModel::UpdateEmpiricalSelAC, this);
        G_tot[0] ++;
        G_acc[1] += ScalingMove(Ginvshape, 0.1, 3, &EmpiricalSelACOmegaModel::AALogProb, &EmpiricalSelACOmegaModel::UpdateEmpiricalSelAC, this);
        G_tot[1] ++;
        return 1.0;
    }

    double MovePsi()  {
        psi_acc[0] += ScalingMove(psi, 1.0, 3, &EmpiricalSelACOmegaModel::AALogProb, &EmpiricalSelACOmegaModel::UpdateEmpiricalSelAC, this);
        psi_tot[0] ++;
        psi_acc[1] += ScalingMove(psi, 0.1, 3, &EmpiricalSelACOmegaModel::AALogProb, &EmpiricalSelACOmegaModel::UpdateEmpiricalSelAC, this);
        psi_tot[1] ++;
        return 1.0;
    }
