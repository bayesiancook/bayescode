...
AAMutSelM2aModel::AAMutSelM2aModel(string datafile, string treefile, double inpi)
    : aahypersuffstat(20) {
...
void AAMutSelM2aModel::Allocate() {
    ...
    sitecodonmatrixarray = new AAMutSelOmegaCodonSubMatrixArray(
        (CodonStateSpace *)codondata->GetStateSpace(), nucmatrix, aafitnessarray, siteomegaarray);

    // sitesubmatrixarray = new
    // MixtureSelector<SubMatrix>(componentcodonmatrixarray,sitealloc);
    // sitecodonmatrixarray = new
    // MixtureSelector<MGOmegaCodonSubMatrix>(componentcodonmatrixarray,sitealloc);
...
void AAMutSelM2aModel::SetNucRatesHyperParameters(
    const std::vector<double> &innucrelratehypercenter, double innucrelratehyperinvconc,
...
void AAMutSelM2aModel::UpdateCodonMatrices() { sitecodonmatrixarray->UpdateCodonMatrices(); }
...
double AAMutSelM2aModel::GetLogLikelihood() const { return phyloprocess->GetLogLikelihood(); }

//
// Suff Stat and suffstatlogprobs
//

const PoissonSuffStatBranchArray *AAMutSelM2aModel::GetLengthPathSuffStatArray() const {
...
double AAMutSelM2aModel::LambdaHyperLogPrior() const { return -lambda / 10; }
...
double AAMutSelM2aModel::AAHyperLogPrior() const { return -aainvconc; }

double AAMutSelM2aModel::AALogPrior() const { return aafitnessarray->GetLogProb(); }

double AAMutSelM2aModel::AALogPrior(int i) const { return aafitnessarray->GetLogProb(i); }
