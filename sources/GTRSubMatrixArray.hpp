...
class GTRSubMatrixArray : public Array<SubMatrix>, public Array<GTRSubMatrix>	{
	...
	public:
	GTRSubMatrixArray(const vector<double>& inrelrate, const Selector<vector<double> >* inprofilearray, int innormalise) : relrate(inrelrate), profilearray(inprofilearray), nstate(inprofilearray->GetVal(0).size()), normalise(innormalise),  matrixarray(inprofilearray->GetSize()) {
        Create();
	}
