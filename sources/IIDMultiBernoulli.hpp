 ...
 *
 * The Nrow*Ncol bidim array is parameterized by a vector of probability parameters (prob, of dimension Nrow).
 ...
 * a_ijk == 1 means that the fitness of amino-acid k at site j is modified (i.e. resampled from gamma prior) upon undergoing a transition to condition i.
 * This occurs with condition-dependent probability prob[i].
 */
...
class BidimIIDMultiBernoulli : public SimpleBidimArray<vector<int> >    {
    ...
    //! constructor, parameterized by number of rows, of columns, dimension of the vectors, and the probability vector
    ...
    //! get number of shift events (number of 1's out of dim) for row i and column j
