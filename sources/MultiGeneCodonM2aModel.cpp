...
MultiGeneCodonM2aModel::MultiGeneCodonM2aModel(string datafile, string intreefile,
    ...
    // 0 : gathering branch lengths across genes and then using gamma suff stats to resample
    // hyperparams
...
void MultiGeneCodonM2aModel::SetMixtureHyperParameters(double inpuromhypermean,
    ...
    if (myid) {
        ...
        for (int gene = 0; gene < GetLocalNgene(); gene++) {
            geneprocess[gene]->SetMixtureParameters(
                (*puromarray)[gene], (*dposomarray)[gene], (*purwarray)[gene], (*poswarray)[gene]);
        }
    }
...
void MultiGeneCodonM2aModel::MoveBranchLengthsHyperParametersIntegrated() {
    ...
    ScalingMove(blhyperinvshape, 1.0, 10,
        &MultiGeneCodonM2aModel::BranchLengthsIntegratedHyperLogProb,
        &MultiGeneCodonM2aModel::NoUpdate, this);
...
double MultiGeneCodonM2aModel::GeneBranchLengthsHyperSuffStatLogProb() const {
    double total = 0;
    for (int j = 0; j < Nbranch; j++) { total += GeneBranchLengthsHyperSuffStatLogProb(j); }
    return total;
}
...
double MultiGeneCodonM2aModel::BranchLengthsHyperScalingMoveIntegrated(double tuning, int nrep) {
    ...
    for (int rep = 0; rep < nrep; rep++) {
        for (int j = 0; j < Nbranch; j++) {
            double deltalogprob =
                -branchlength->GetLogProb(j) - GeneBranchLengthsHyperSuffStatLogProb(j);
            double m = tuning * (Random::Uniform() - 0.5);
            double e = exp(m);
            (*branchlength)[j] *= e;
            deltalogprob += branchlength->GetLogProb(j) + GeneBranchLengthsHyperSuffStatLogProb(j);
            deltalogprob += m;
...
int MultiGeneCodonM2aModel::PoswCompMove(double tuning) {
    ...
    double m = tuning * (Random::Uniform() - 0.5);
    double e = exp(m);
    if (poswhypermean * e > 1.0) { return 0; }
    poswhypermean *= e;
    poswhyperinvconc *= e;
...
void MultiGeneCodonM2aModel::CollectGeneBranchLengthsSuffStat() {
    for (int gene = 0; gene < GetLocalNgene(); gene++) {
        ...
        (*lengthpathsuffstattreearray)[gene].BranchArray<PoissonSuffStat>::Copy(
...
void MultiGeneCodonM2aModel::SlaveReceiveNucRatesHyperParameters() {
    ...
    for (int gene = 0; gene < GetLocalNgene(); gene++) {
        geneprocess[gene]->SetNucRatesHyperParameters(
            nucrelratehypercenter, nucrelratehyperinvconc, nucstathypercenter, nucstathyperinvconc);
    }
}
...
void MultiGeneCodonM2aModel::SlaveReceiveMixtureHyperParameters() {
    ...
    for (int gene = 0; gene < GetLocalNgene(); gene++) {
        geneprocess[gene]->SetMixtureHyperParameters(puromhypermean, puromhyperinvconc,
            dposomhypermean, dposomhyperinvshape, pi, purwhypermean, purwhyperinvconc,
