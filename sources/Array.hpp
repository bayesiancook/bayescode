 ...
 * The SimpleArray class template provides a more explicit implementation (by encapsulating a std::vector<T>, see
 * SimpleArray).
...
class SimpleArray : public Array<T> {
  public:
    ...
    //! Apply a permutation over the arguments, such that entry indexed by i after the call is equal to entry formely indexed
    //! by permut[i]
 ...
 *
 * This Selector<T> maintains a pointer over an array of (say K) components and an array of (say N) integer allocation
 * variables.
 * Then, for any index i, GetVal(i) returns a reference to the component to which item i is allocated (i.e.
 * components[alloc[i]])
 */
...
class MixtureSelector : public Selector<T> {
  public:
    ...
    //! GetVal(i) (for i in 0..alloc->GetSize()-1) returns a reference to the component to which item i is allocated (i.e.
    //! components[alloc[i]])
