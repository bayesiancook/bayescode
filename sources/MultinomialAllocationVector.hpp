 ...
 *
 * MultinomialAllocationVector specifies the probability distribution of the multinomial allocation of N items to the K components of the finite mixture.
...
class MultinomialAllocationVector : public SimpleArray<int> {
    ...
    //! Constructor (parameterized by number of items (N) and mixture weights (vector of dimension K)
    ...
    //! resample allocation of item i based on an externally given vector of posterior probabilities
    void GibbsResample(int i, const vector<double>& postprob)   {
        (*this)[i] = Random::DrawFromDiscreteDistribution(postprob);
    }

    //! resample all allocations based on an externally given array of item-specific posterior probabilities
