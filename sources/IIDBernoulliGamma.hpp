...
/**
 * \brief A sufficient statistic for a collection of random variables, from a mixture of a point mass at 0 and a Gamma distribution (see IIDBernoulliGamma)
 */
...
class BernoulliGammaSuffStat : public SuffStat	{
    ...
    //! return log prob of suff stats, as a function of parameters of the mixture and the Gamma distribution
	double GetLogProb(double pi, double alpha, double beta) const {
        double logbern = n0 * log(1-pi) + n1 * log(pi);
        double loggamma = n1 * (alpha*log(beta) - Random::logGamma(alpha) + (alpha-1)*sumlog - beta*sum);
...
/**
 * \brief An array of IID random variables, from a mixture of a point mass at 0 (weight 1-pi) and a Gamma distribution (weight pi)
 */
...
class IIDBernoulliGamma : public SimpleArray<double> {
    ...
    //! constructor, parameterized by size, weight parameter pi of the mixture, and parameters of the Gamma (alpha and beta)
    IIDBernoulliGamma(int insize, double inpi, double inalpha, double inbeta) : SimpleArray<double>(insize), pi(inpi), alpha(inalpha), beta(inbeta)  {
        Sample();
    }
