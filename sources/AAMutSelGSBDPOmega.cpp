#include <cmath>
#include <fstream>
#include "AAMutSelGSBDPOmegaModel.hpp"
#include "Chain.hpp"
using namespace std;

/**
 * \brief Chain object for running an MCMC under AAMutSelGSBDPOmegaModel
 */

class AAMutSelGSBDPOmegaChain : public Chain {
  private:
    // Chain parameters
  ...
  public:
    AAMutSelGSBDPOmegaModel *GetModel() { return static_cast<AAMutSelGSBDPOmegaModel *>(model); }

    string GetModelType() override { return modeltype; }

    AAMutSelGSBDPOmegaChain(string indatafile, string intreefile, int inomegamode, int inomegaprior,
                            double indposompi, double indposomhypermean,
                            double indposomhyperinvshape, double inmaxdposom,
                            int inNcat, int inGcat, int inevery,
                            int inuntil, string inname, int force)
        : modeltype("AAMUTSELSBDPOMEGA"),
          datafile(indatafile),
          treefile(intreefile),
    ...
    AAMutSelGSBDPOmegaChain(string filename) {
        name = filename;
        Open();
        Save();
    }
    ...
    void New(int force) override {
        cerr << "new model\n";
        model =
            new AAMutSelGSBDPOmegaModel(datafile, treefile, omegamode, omegaprior, Ncat, Gcat);
        if (omegaprior != 0) {
    ...
    void Open() override {
        ...
        if (modeltype == "AAMUTSELSBDPOMEGA") {
            model = new AAMutSelGSBDPOmegaModel(datafile, treefile, omegamode, omegaprior, Ncat, Gcat);
            if (omegaprior != 0) {
                GetModel()->SetMaxDPosOm(maxdposom);
...
int main(int argc, char *argv[]) {
    string name = "";
    AAMutSelGSBDPOmegaChain *chain = 0;

    // starting a chain from existing files
    if (argc == 2 && argv[1][0] != '-') {
        name = argv[1];
        chain = new AAMutSelGSBDPOmegaChain(name);
    }
    ...
    // new chain
        ...
        chain = new AAMutSelGSBDPOmegaChain(datafile, treefile, omegamode, omegaprior, dposompi,
                                            dposomhypermean, dposomhyperinvshape, maxdposom, Ncat, Gcat,
