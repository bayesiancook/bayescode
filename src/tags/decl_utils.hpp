...
template <class... Tags, class ProcessingInfo, class Target, class... Args>
void declare(ProcessingInfo& processing_info, string name, Target& target, Args&&... args) {
    auto decl_info = make_decl_info<Tags...>(target, name);
    ProcessingInfo::processing::forward_declaration(
...
namespace decl_utils {  // namespace to hide helpers
    ...
    class End {
      public:
        template <class ProcessingInfo, class DeclInfo, class... Args>
        static void forward_declaration(ProcessingInfo prinfo, DeclInfo declinfo, Args&&... args) {
