...
template <class T>
unsigned int MPISize(const T &t) {
    return t.GetMPISize();
}
...
template <class T>
MPIBuffer &operator<<(MPIBuffer &buffer, const T &t) {
    buffer.Put(t);
    return buffer;
};

template <class T>
const MPIBuffer &operator>>(const MPIBuffer &buffer, T &t) {
    buffer.Get(t);
    return buffer;
}
...
template <class T>
T &operator+=(T &t, const MPIBuffer &buffer) {
    t.Add(buffer);
    return t;
}
