...
std::vector<Tree::NodeIndex> TaxonSet::get_index_table(const Tree *tree) const {
    // The taxon given the node
    std::vector<Tree::NodeIndex> ret(tree->nb_nodes(), -1);
    for (size_t node = 0; node < tree->nb_nodes(); node++) {
        if (tree->is_leaf(node)) {
            if (tree->node_name(node).empty()) { FAIL("Leaf has no name") }
...
int TaxonSet::GetTaxonIndexWithIncompleteName(string const &taxname) const {
    int found = -1;
    for (int i = 0; i < Ntaxa; i++) {
