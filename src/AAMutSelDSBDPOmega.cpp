...
public:
    ...
    ValueArg<int> ncat{"", "ncat", "truncation of the first-level stick-breaking process", false, 100, "int", cmd};
    ValueArg<int> basencat{"", "basencat", "truncation of the second-level stick-breaking process",
                           false, 1, "int", cmd};
    SwitchArg freeomega{"", "freeomega", "omega is allowed to vary with shrinkage (otherwise set to 1)", cmd, false};
    SwitchArg mixomega{"", "mixomega", "the prior of omega is a mix of (1-pi) at 1 and pi at 1+d, "
                                       "with d~Gamma(dposomhypermean,dposomhyperinvshape)", cmd, false};
    ValueArg<double> dposompi{"", "dposompi", "proportion of positively selected sites (if -mixomega is switched on)",
                              false, 0.1, "double", cmd};
    ValueArg<double> dposomhypermean{"", "dposomhypermean", "inverse shape of the gamma distribution "
                                                            "(if -mixomega is switched on)", false, 1.0, "double", cmd};
    ValueArg<double> dposomhyperinvshape{"", "dposomhyperinvshape", "inverse shape of the gamma distribution "
...
int main(int argc, char *argv[]) {
    ...
    if (cmd.resume_from_checkpoint()) {
        ...
        cmd.parse();
        chain_driver = new ChainDriver(cmd.chain_name(), args.every.getValue(), args.until.getValue());
        model = new AAMutSelDSBDPOmegaModel(args.alignment.getValue(), args.treefile.getValue(),
                                            ...
                                            aamutsel_args.dposomhyperinvshape.getValue(),
