...
class ReadBranchOmegaMutSelArgParse : public ReadArgParse {
  public:
    explicit ReadBranchOmegaMutSelArgParse(CmdLine &cmd) : ReadArgParse(cmd) {}
...
int main(int argc, char *argv[]) {
    ...
    ReadBranchOmegaMutSelArgParse read_args(cmd);
    cmd.parse(argc, argv);
    ...
    if (read_args.GetPpred()) {
        ...
        recompute_trace<BranchOmegaNeSiteMutSelModel>(*model, cr, chain_name, every, size);
        ...
        stats_posterior<BranchOmegaNeSiteMutSelModel>(*model, cr, every, size);
    }
}